(function () {
    ...
    function cj(a, b) {
        ...
        return e.domain = function (b) {
            return arguments.length ? (a = b.filter(
                function (a) {
                    return!isNaN(a)
    ...
    var p = {svg:"http://www.w3.org/2000/svg", xhtml:"http://www.w3.org/1999/xhtml", xlink:"http://www.w3.org/1999/xlink", xml:"http://www.w3.org/XML/1998/namespace", xmlns:"http://www.w3.org/2000/xmlns/"};
    d3.ns = {prefix:p, qualify:function (a) {
        var b = a.indexOf(":");
        return b < 0 ? a in p ? {space:p[a], local:a} : a : {space:p[a.substring(0, b)], local:a.substring(b + 1)}
    ...
    var s = /(?:([^{])?([<>=^]))?([+\- ])?(#)?(0)?([0-9]+)?(,)?(\.[0-9]+)?([a-zA-Z%])?/, t = {g:function (a, b) {
    ...
    var z = H(2), A = H(3), B = {linear:function () {
        return G
    ...
    d3.ease = function (a) {
        ...
        P.lastIndex = 0;
        for (d = 0; c = P.exec(b); ++d)c.index && h.push(b.substring(f, g = c.index)), i.push({i:h.length, x:c[0]}), h.push(null), f = P.lastIndex;
    ...
    bi.style = function (a, b, c) {
        ...
        for (var a = -1, b = this.length; ++a < b;)for (var c = this[a], d = c.length - 1, e = c[d], f; --d >= 0;)if (f = c[d])e && e.parentNode.insertBefore(f, e), e = f;
        return this
    }, bi.sort = function (a) {
    ...
    var cv = {linear:cw, "step-before":cx, "step-after":cy, basis:cE, "basis-open":cF, "basis-closed":cG, bundle:cH, cardinal:cB, "cardinal-open":cz, "cardinal-closed":cA, monotone:cQ}, cJ = [0, 2 / 3, 1 / 3, 0], cK = [0, 1 / 3, 2 / 3, 0], cL = [0, 1 / 6, 2 / 3, 1 / 6];
    ...
    d3.svg.axis = function () {
        function j(j) {
            j.each(function (k, l, m) {
                ...
                switch (b) {
                    case"bottom":
                        A = dh, v.attr("x2", 0).attr("y2", d), z.select("line").attr("x2", 0).attr("y2", c), z.select("text").attr("x", 0).attr("y", Math.max(c, 0) + f).attr("dy", ".71em").attr("text-anchor", "middle"), E.attr("d", "M" + B[0] + "," + e + "V0H" + B[1] + "V" + e);
                        break;
                    case"top":
                        A = dh, v.attr("x2", 0).attr("y2", -d), z.select("line").attr("x2", 0).attr("y2", -c), z.select("text").attr("x", 0).attr("y", -(Math.max(c, 0) + f)).attr("dy", "0em").attr("text-anchor", "middle"), E.attr("d", "M" + B[0] + "," + -e + "V0H" + B[1] + "V" + -e);
                        break;
                    case"left":
                        A = di, v.attr("x2", -d).attr("y2", 0), z.select("line").attr("x2", -c).attr("y2", 0), z.select("text").attr("x", -(Math.max(c, 0) + f)).attr("y", 0).attr("dy", ".32em").attr("text-anchor", "end"), E.attr("d", "M" + -e + "," + B[0] + "H0V" + B[1] + "H" + -e);
                        break;
                    case"right":
                        A = di, v.attr("x2", d).attr("y2", 0), z.select("line").attr("x2", c).attr("y2", 0), z.select("text").attr("x", Math.max(c, 0) + f).attr("y", 0).attr("dy", ".32em").attr("text-anchor", "start"), E.attr("d", "M" + e + "," + B[0] + "H0V" + B[1] + "H" + e)
                }
        ...
        function e(a) {
            ...
            a.each(function () {
                ...
                h.enter().append("rect").attr("class", "background").style("visibility", "hidden").style("pointer-events", "all").style("cursor", "crosshair"), i.enter().append("rect").attr("class", "extent").style("cursor", "move"), j.enter().append("rect").attr("class",
                    function (a) {
                        return"resize " + a
                    }).attr("width", 6).attr("height", 6).style("visibility", "hidden").style("pointer-events", e.empty() ? "none" : "all").style("cursor", function (a) {
                        return dC[a]
                    }), j.exit().remove(), b && (k = bP(b), h.attr("x", k[0]).attr("width", k[1] - k[0]), dv(a, d)), c && (k = bP(c), h.attr("y", k[0]).attr("height", k[1] - k[0]), dw(a, d))
            })
        }
        ...
        var a = d3.dispatch("brushstart", "brush", "brushend"), b, c, d = [
            [0, 0],
            [0, 0]
        ];
    ...
    var dk, dl, dm, dn, dp, dq, dr, ds, dt, du, dC = {n:"ns-resize", e:"ew-resize", s:"ns-resize", w:"ew-resize", nw:"nwse-resize", ne:"nesw-resize", se:"nwse-resize", sw:"nesw-resize"};
    ...
    var dP, dQ, dR, dS = {}, dT = 0, dU, dV, dW, dX, dY, dZ, d$, eh = [
        [-Infinity, Infinity],
        [-Infinity, Infinity],
        [-Infinity, Infinity]
    ]
